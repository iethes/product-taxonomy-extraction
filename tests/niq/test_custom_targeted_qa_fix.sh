...
future_scope_brief='## Targeted QA Fix Brief
...
[[ "$(has_real_brief "$future_scope_brief")" == "false" ]] || fail "'not executed this session' disclaimer must be treated as no-real-brief, even with a filled Verdict line"
...
scoped_brief='## Targeted QA Fix Brief
...
[[ "$(has_real_brief "$scoped_brief")" == "true" ]] || fail "disclaimer text outside the Brief section must not suppress a real brief"
...
prompt=$(build_auto_discovery_prompt "makanankucing" "9_makanankucing_my_daily" "makanankucing_my" "makanankucing.makanankucing_my" "200")
