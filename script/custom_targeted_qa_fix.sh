...
resolve_category_file() {
  local category="$1"
  local candidate="docs/categories/${category}.md"
  if [[ -f "$candidate" ]]; then
    echo "$candidate"
    return 0
  fi
...
has_real_brief() {
  ...
  local brief_line
  brief_line=$(grep -n "^## Targeted QA Fix Brief" "$category_file" 2>/dev/null | head -1 | cut -d: -f1)
  ...
  local section
  section=$(awk -v start="$brief_line" 'NR > start && /^## / { exit } NR >= start { print }' "$category_file")
