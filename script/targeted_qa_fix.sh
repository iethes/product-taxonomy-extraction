...
has_real_brief() {
  ...
  # Some Verdicts are a deferral disclaimer ("not yet assessed", "none written yet — auto-discovery
  # mode is correct") rather than a template placeholder — they read as "real" to a bare `{` check
  # but brief mode then either has nothing to execute (blocks every run) or covers a narrower scope
  # than qa_report.sh's whole-category gate check (fails every run). Found via 2026-08-01 audit of
  # shopee_th_detergent/shopee_th_moisturizer_for_body. ponytail: known-phrase list, not exhaustive —
  # extend it if a future category's disclaimer wording slips through.
  local section
  section=$(awk -v start="$brief_line" 'NR > start && /^## / { exit } NR >= start { print }' <<< "$brief_markdown")
  if grep -qiE "not yet assessed|none written yet|not executed this session" <<< "$section"; then
    echo "false"
    return
  fi
  local verdict_line
...
Policy (management decision): a product that is simply irrelevant/out-of-category for ${table} — genuinely a
different product type, sitting in this source table's raw data for reasons upstream of this pipeline — is
...
any master_table, not just this one) over every taxonomy_id currently mapped to ${table}:
SELECT pt.taxonomy_id, pt.canonical_name, dup.taxonomy_id AS duplicate_of, dup.canonical_name AS duplicate_canonical_name
FROM \`${PROJECT}.magpie_reference.product_taxonomy\` pt
JOIN \`${PROJECT}.magpie_reference.product_taxonomy_map\` m ON m.taxonomy_id = pt.taxonomy_id AND m.master_table = '${table}'
