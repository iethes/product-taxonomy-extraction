...
- **"All variant" / "All size"** entries are not acceptable as canonical names. Use:
  ...
  - `is_multi_size=TRUE` + a meaningful name if it genuinely covers multiple sizes — same rule for
    `is_multi_variant=TRUE`
- **"Multiple Sizes" / "Multiple Variants"** in `canonical_name` are also not acceptable, **even when**
  `is_multi_size=TRUE`/`is_multi_variant=TRUE` is correctly set (superseding the shopee_th_softdrink precedent
  below, which sanctioned this exact phrasing — corrected 2026-07-22). The `is_multi_size`/`is_multi_variant`
  column already conveys that semantic; restating it as generic text in `canonical_name` is the same
  ungrounded-stub problem as "All variant"/"All size" — the flag doesn't excuse it. Use a real, meaningful
  name for the entry instead (e.g. the brand + product line alone, with no size/variant suffix at all).
...
| Jul 22 2026 | universal | **"Multiple Sizes"/"Multiple Variants" banned unconditionally, superseding the shopee_th_softdrink precedent above.** That precedent sanctioned this phrasing when paired with `is_multi_size=TRUE`/`is_multi_variant=TRUE`; corrected same-day after review — the flag column already conveys that semantic, so the text is the same ungrounded-stub defect as "All variant"/"All size" regardless of whether the flag is set. Existing entries using this phrasing (including any from the shopee_th_softdrink category) need correcting, not exempting. Also: `docs/quality-standards.md`'s D5 pack-count-mismatch query never actually checked for English "buy N get M" phrasing despite this doc's own §1 table listing "buy 1 get 1" as pack_count=2 — found via product `16254994627`, whose "Buy 1 Get 1" sku_name text was missed because only Thai patterns and the digit form `1\+1` were in the regex. Fixed the regex (also added `LOWER()` — sku_name casing varies and the pattern is case-sensitive without it) and operationalized both checks into `script/targeted_qa_fix.sh`'s Tier 1 sweep, which had never actually run either check before (D5's query existed only as a manual snippet in quality-standards.md). |
