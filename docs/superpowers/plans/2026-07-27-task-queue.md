...
- `QUEUE_TABLE` is computed as a plain, side-effect-free top-level assignment (`QUEUE_TABLE="${QUEUE_SCHEMA:-public}.task_queue"`) at the top of `queue_worker.sh` and `queue_ctl.sh` — NOT inside `load_env.sh`'s `main()`-gated env loading, and NOT gated behind sourcing `.env` at all. It must be available the moment either script is sourced (including when sourced for pure-function testing, before `.env` has ever been read), so every `build_*_sql`/query-building function can reference it unconditionally. **`main()` in both files additionally recomputes `QUEUE_TABLE` with this same expression immediately after `source load_env.sh`.** The top-level assignment alone is not sufficient for actual invocation: it runs at parse time, before `.env` has been loaded, so it locks in whatever `QUEUE_SCHEMA` (if any) already happened to be in the process environment at that point — correct by accident under the documented `source script/load_env.sh; script/queue_ctl.sh ...` interactive pattern (which pre-exports `QUEUE_SCHEMA` before the script even starts), but silently wrong (falls back to `public`) under any clean-environment invocation — systemd, cron, or a bare `./script/queue_worker.sh` — where `.env` is only ever loaded by `main()`'s own `source load_env.sh` call, moments after the stale top-level value was already computed. Keep both assignments: the top-level one for pure-function test sourcing (`main()` never runs under test), the in-`main()` one for every real invocation.
