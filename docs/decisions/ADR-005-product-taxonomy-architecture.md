...
- **`flag_GWP = TRUE` products are NOT excluded from any pass** (revised 2026-07-17, superseding the original "excluded from all passes" decision below) — their GMV is still zeroed in brand ranking and product threshold calculations, but they now go through extraction and enter `product_taxonomy_map` like any other in-scope product. Original rationale for exclusion is not recorded in this ADR; the exclusion was found to be silently dropping identifiable, in-scope GWP products from coverage during the `shopee_th_suncare` v2 rebuild (2026-07-17)
