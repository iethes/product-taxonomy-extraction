...
| LLM Pass 1 | ✅ Complete |
| LLM Pass 2 | ✅ Complete |
| GMV Coverage | 96.2% (2026-06, after 2026-07-29 top-up session; unchanged after 2nd top-up session, see QA History — remaining worklist is the documented exclusion set, not a gap) |
| Last run | 2026-07-29 (Top-up coverage session, 2nd — no-op, confirmed remaining worklist is already-excluded products; prior: Top-up coverage session; prior: Full Rebuild, first-run) |
...
| 2026-07-29 | Post-write QA gates | First gate run found 2 placeholder-leak violations: `SKU-213859`/`SKU-213862`, both literally containing "Multiple Variants" in `canonical_name`, sourced from 2 reseller listings genuinely offering buyer's-choice between two *different* brands (Tide vs. Gain) in one SKU | Deleted both taxonomy entries and their 2 map rows — matches the `shopee_th_softdrink` precedent for multi-brand buyer-choice listings (leave unmapped, not a forced/generic name) rather than rename into something misleading |
...
| 2026-07-29 | Top-up coverage session (2nd) | Wrapper's live pre-check reported 31 rows still unmapped within the top-95%-cumulative-GMV worklist. Re-ran STEP 0's worklist query live (per this session's instruction not to trust the pre-check number) and got 31 rows / 15 distinct products. All 15 fall in categories this category's own Scope section marks out-of-scope, or match ambiguous buyer-choice listings this category has already set precedent on: Finish + P&G JOY (dishwasher gel/powder — out of scope per "Dish soap/washing-up liquid"; NS Hardware "Supersteam Happydish" dish-washing agent is a third instance of the same out-of-scope category, not individually named in the prior session's abbreviated "Finish, P&G Joy" list but covered by the same Scope rule), ANGGUN (body soap, out of scope), Comfort (pure fabric softener with no wash function, out of scope per Scope section), Karcher (appliance descaler, out of scope — not a laundry product), DChemie (unlabeled industrial soda ash, brand/product unresolvable), 99 Megamart Tide/Gain ×2 product_ids + Arm & Hammer (buyer-choice "Multiple Variant(s)" listings — can't determine what the buyer receives, same placeholder-leak risk as this category's own SKU-213859/213862 precedent), P&G Lenor (scent booster beads, not a detergent). Totals (15 products / 31 rows) match the prior top-up session's own tally exactly, and every category-level match is consistent with that session's documented exclusions — softening the claim to reasoning-based rather than asserting a verified 1:1 product identity, since the prior entry named examples per category rather than enumerating all 15 explicitly. The two highest-GMV rows deserve their own verdict rather than inheriting the precedent wholesale: Poddo "Home Care Bundle" (GMV 9,966, Shopee Mall) and Walch SG "Earth Day Bundle" (GMV 512+224+96 across 3 models, Shopee Mall) are both official-store Mall listings — exactly the "high-GMV Mall-seller listing might be genuinely miscategorized" case this session was warned to check individually, not filter out by keyword. Both bundle multiple product categories in one SKU (Poddo: laundry pods + liquid detergent + hand wash + dishwashing; Walch: Dynamo/Arfum capsules + Fab dishwashing detergent) with no single derivable line/size/pack for the laundry component alone — matches this category's own documented Scope exclusion ("General household cleaning equipment... that happens to appear under a Mall merchant selling in-scope products") and the Pass 1+2 "Poddo x3/Dynamo" cross-category-kit gate decision, evaluated fresh against product-lifecycle.md §4.2 this session, not assumed from the prior list. Confirmed via `product_taxonomy_map` that none of these 15 `product_id`s have a row under *any* `master_table`, consistent with genuine exclusion rather than a missed product. This worklist is not a new coverage gap — "unmapped" is indistinguishable from "excluded" in a raw `canonical_name IS NULL` query. No SKU block claimed, no rows written — bulk-minting taxonomy entries for these would have contradicted this category's own documented Scope section and precedent. | No action taken. Coverage remains 96.2% (2026-06), 1,305 LLM map rows, 1,321 dict entries in SKU-213207–215615 (0 orphans, both `sku_block_registry` claims intact) — live state matches this file's documentation exactly, ruling out the pipeline-wide HUMAN-row data-loss incident tracked in project memory for this category. Post-check QA gates (no `--skip-coexistence`, exact queries from `docs/headless-runbook.md`): G1 dual-mapped=0, G2 HUMAN+LLM coexistence=0, placeholder-leak=0, structured-fields NULL%=0, G4 cross-category (this category's 1,305 mapped product_ids checked for a second `master_table` row anywhere in `product_taxonomy_map`)=0. |
