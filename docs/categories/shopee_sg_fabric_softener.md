...
| LLM Pass 1 | ✅ Complete |
| LLM Pass 2 | ✅ Complete |
| GMV Coverage | 89.19% of total category GMV (2026-06); 49 Rule-A in-scope products remain NULL, all confirmed out-of-scope (see QA History) |
| Last run | 2026-07-29 |
...
| SKU-211007–211206 | Top-up session 2026-07-29 (bulk-first): 55 taxonomy entries (SKU-211007–211061), 84 products mapped (59 to new entries + 25 reused against existing entries); 145 slots unused |
...
| 2026-07-29 | Top-up (bulk-first) | Wrapper's pre-check quoted 191 products; re-ran the live STEP 0 query as instructed (product-grain, `--max_rows=100000`) and got a genuinely different, much larger number: 191 model-grain rows / 133 distinct products still NULL within the 95%-cumulative-GMV (GWP-zeroed) threshold — the category had accumulated new listings since the same-day top-up above, and this confirms that top-up's own live-worklist number can go stale within hours. First CSV export of this query silently truncated to 99 rows despite `--max_rows=100000` (a CTE-name typo caused a query failure that returned a stale/incomplete prior result to the file instead of erroring visibly) — re-ran and got the correct 192-row (191+header) file, cross-checked against a `COUNT(*)` query before trusting it. | Classified all 133 products via bulk text-matching (sku_name + existing `product_brand_map` brand assignments), never image reads — this session prioritized coverage over precision per the headless runbook. 49 confirmed OOS (ironing/wrinkle spray, starch spray, Kispray/Rapika, laundry pods/capsules, Cosway dish soap/bathroom cleaner/ambiguous fabric-wash listings, stain/spot remover, foot softener, ironing-board/travel-board covers, wool dryer ball [physical accessory, not chemical], a skincare-toner contamination row, a dryer-machine fragrance flacon, a laundry color-booster, 3 ambiguous multi-option/multi-variant buyer-choice listings that can't be resolved from text, and one Cosway listing already known contaminated). 84 in-scope: 25 bulk-matched to existing SKU-208xxx/SKU-210607 entries (exact or near-exact brand+line+size+pack text match — e.g. `Comfort Concentrate Fabric Softener 1.5L x6`→SKU-208236, `Ecover Fabric Softener Gardenia & Vanilla 750ml`→SKU-208233), 59 minted across 55 new entries (grouped by brand+line+size+pack before minting, e.g. two separate `Downy Peach Blossom 1.1L` cartons-of-9 listings both routed to one new entry) — genuinely ambiguous multi-size/multi-variant listings (buyer picks from several stated sizes/scents with no way to tell which) got `is_multi_size`/`is_multi_variant` catch-all entries rather than a forced single size, per D4/D3 rules; two brands (SHIRO-line "Fadric Hotel" 500ml softener, and a "SHIRO White Tea" listing) had no existing `product_brand_map` row at all — routed via `brand_dict` text lookup directly since taxonomy mapping doesn't require `product_brand_map` agreement. Claimed SKU-211007–211206 (200-slot block; 55 used, 145 unused). Post-write QA gates (G1 LLM-scoped dual-mapped, G2 HUMAN+LLM coexistence, placeholder-leak, structured-fields-NULL%, G5 provenance, G6 brand_mismatch) all 0 — reran WITHOUT `--skip-coexistence` since this category already shipped once. Live worklist re-verified post-write: 49 products remain NULL, exactly matching the 49 rows classified OOS (no coverage miss). Category GMV coverage rose 87.08% → 89.19%. Per-row precision (exact on-label product-line wording, sub-line vs. variant field placement) was not verified against product images this session — flagging for a future `targeted_qa_fix.sh` pass, same as the existing long-tail-brand gap noted below. |
...
| Source | Count | Notes |
|--------|-------|-------|
| LLM | 443 | Pass 1 + Pass 2 (358) + top-up session 2026-07-29 (+1, SKU-210607) + top-up session 2026-07-29 bulk-first (+84, SKU-211007–211061 range) |
| HUMAN | 0 | No prior keyword-seed pass ran for this table |
| NULL (unmapped, in-scope) | 49 | All confirmed out-of-scope product-type contamination (ironing spray, laundry pods, stain remover, Assos Active Wear Cleanser [detergent], skincare-toner contamination, wool dryer ball, ambiguous multi-option listings, etc.) — see Scope and QA History |
