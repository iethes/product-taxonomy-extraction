...
| 2026-07-29 | Top-up | Pre-run verification: prompt's claimed 95-row worklist matched live re-query exactly (95 rows / 66 distinct products); live `product_taxonomy_map`/`product_taxonomy` state (752 LLM rows, 68 dictionary entries) matched this doc's own documented counts exactly — this category is **not** a data-loss victim (unlike several `th_*` and other SG categories flagged in the pipeline-wide `HUMAN`-row-loss incident); safe to proceed as a genuine top-up | Confirmed via direct row-count comparison before writing; no orphaned map rows found |
| 2026-07-29 | Top-up | Of the 66 distinct products in the live gap, 19 were genuinely out-of-scope on individual inspection, not a coverage miss: 3 NOMIEO carton listings (`17001573981`, `15117514467`, `44950232641`, 18 rows) offer both toilet-roll-count and facial-tissue variants in one listing with no per-model_id signal in the source table to tell which was purchased — confirmed via direct image read (listing shows both "27 Rolls Toilet Paper" and "32 Packs Facial Tissue" as separate SKUs in one carton ad); left NULL rather than guess, per this doc's existing NOMIEO caution. `Homestead` (`42675309025`) and `Home.Essentials`/Donglan (`26904947984`) sku_names were ambiguous between roll and soft-pack tissue ("175×175mm"/"160×175mm" sheet-dimension format, "Pumping paper" i.e. 抽纸) — image-confirmed both are flat pull-pack facial tissue, not rolls; left NULL. Remaining 14 products (Zappy ×2, Soul Maseuko ×2, Botare-branded wipes, K Sparks, NUFRESH ×2, Carapiu, Cherl93, Manhua-branded wipes, AIDEAL, Jie Rou C&S-branded wipes) are wet-wipes/flushable-tissue format under this doc's existing wet-wipes exclusion — several share a brand with an in-scope roll entry (Botare, Jie Rou C&S, Manhua) but the per-product type gate applies regardless of brand, per this doc's NOMIEO note | 19 products (38 worklist rows) correctly left `taxonomy_id IS NULL`; documented here so a future session doesn't mistake this for an unresolved gap |
| 2026-07-29 | Top-up | Remaining 47 products resolved: 3 mapped to the existing Manhua catch-all (`SKU-201382`); 11 to new entries under already-existing `brand_dict` brands (NOMIEO, Cheerful, KCA, Pulppy, Soiselle, Qing Feng, NooTrees, Prefer, Pinse, Royal Gold, Tempo-new-line, Peri); 9 to new entries under newly-minted `brand_dict` SG brands (Love Brands, ONES, Bamboolove, FreshTouch, Buyela, HOLLA, Heartprint, Watsons Flower [private label, distinct from the Watsons retailer entity], Ikuma — `BRD-SG-14523`–`BRD-SG-14531`); 13 genuinely unbranded/generic-reseller products (no product-level brand text, only a merchant name) routed to one shared `BRD-UNBRANDED` catch-all rather than inventing a merchant-name-as-brand entry, per `llm-extraction-rules.md` §11 (never derive brand from `merchant_name`). Result: +22 taxonomy entries (`SKU-215854`–`SKU-215875`), +9 brand_dict entries, +47 map rows (752→799 LLM, 0 HUMAN). GMV coverage 75.0%→83.0%. G1/G2/G5 and an orphan check all 0 post-write. Per-row precision (e.g. splitting the 9 `is_multi_size=TRUE` reseller-assorted catch-alls by actual size/pack once more listings accumulate) deferred to `targeted_qa_fix.sh`, consistent with this category's Full Rebuild precedent | Live worklist re-query after write: 38 rows / 19 products remain, all in the confirmed-OOS set above — gap is genuinely closed, not under-resolved |
...
| Source | Count | Notes |
|--------|-------|-------|
| LLM | 799 | Pass 1 (58 official-store products, 46 taxonomy entries) + Pass 2 (694 products: 16 no-official-store-brand catch-alls, 6 Pass-1-brand reseller catch-alls, blank-brand text-detected routing) + 2026-07-29 top-up (47 products, +22 taxonomy entries) |
| HUMAN | 0 | None existed before this session |
| NULL (unmapped) | 3,470 | Mostly the blank-brand long tail plus below-95%-threshold-brand, out-of-category (facial tissue/wet wipes), and out-of-category products |
