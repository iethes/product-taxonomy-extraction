...
| Source | Count | Notes |
|--------|-------|-------|
| LLM | 1,595 | 1,331 from sessions #1/#2 (2026-07-16 Pass 1+2 + first top-up) + 264 from this session (session #3): 109 reuse-matched + 155 newly minted |
| HUMAN | 0 | All `HUMAN` rows for this category — and platform-wide across every category in `product_taxonomy_map` (137,228 `LLM` rows, 0 `HUMAN`) — were gone at the start of this session, undocumented. See session #3's QA History row above; not caused by this session, reported as a data-integrity anomaly for human follow-up. |
| NULL (unmapped) | remainder of distinct products in `master_clean_niq.shopee_sg_toothpaste` | GMV coverage for month 2026-06 now 96.77% (1,552 of 9,116 distinct products), exceeding the ≥90% NULL-coverage-pass target in `docs/quality-standards.md` §6. Live worklist re-check after this session's writes: 9 rows (7 distinct products) remain — all 7 are confirmed-OOS-left-NULL by design (4 newly confirmed this session + 3 carried over from sessions #1/#2), not a defect. |

**QA gate results (session #3, run WITHOUT `--skip-coexistence` per `docs/headless-runbook.md`):**

| Gate | Result | Note |
|------|--------|------|
| G1 dual-mapped LLM | 0 | Pass |
| G2 HUMAN+LLM coexistence | 0 | Pass — trivially true now since `HUMAN` count is 0 platform-wide (see anomaly above), not because a delete ran this session |
| G3 placeholder-leak | 450 rows / **17 distinct entries** | The 17 distinct entries are unchanged — the same pre-existing `SKU-072520`–`072536` catch-alls flagged in sessions #1/#2 and already deferred to a future `targeted_qa_fix.sh` pass. The raw row-count (450, up from the row-count-based number sessions #1/#2 reported) reflects `run_qa_gates`'s Gate 3 query joining through `product_taxonomy_map` without `DISTINCT` — it fans out per mapped product exactly the way Gate 4's query did before its 2026-07-16 fix (see `docs/headless-runbook.md`'s QA-gate-as-code comments); this session added 30 new map rows onto 8 of those 17 pre-existing entries via legitimate reuse-before-mint, which is what moved the row-count, not new violating entries. No entry minted this session uses the banned phrasing. |
| G4 structured-fields-NULL% | 0% | Pass |
| G5 provenance | 0 missing | Pass — all 264 new map rows and 97 new taxonomy rows have `meta_agent='CLAUDE_CODE'`, `source='LLM'` |
