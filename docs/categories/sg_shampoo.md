...
| 2026-07-23 | Automated Taxonomy Review (targeted_qa_fix auto-discovery) | Worklist: 2,377 never-reviewed/unconfident `product_taxonomy` entries mapped to this category. Tier 1 SQL sweep flagged: `stub_leak` 182 (banned "(all variants)"/"Multiple Variants" phrasing, incl. 51 pre-existing keyword-seed catch-alls and 129 of the 2026-07-15 Pass-1 catch-alls the earlier top-up session explicitly deferred), `brand_casing_mismatch` 69 (e.g. `Lux`→`LUX`, `anillo` case), `duplicate_brand` 18 flagged/5 genuine (`LebeL LebeL...`, `Schwarzkopf Schwarzkopf...`), `wrong_field_order` 83 flagged/68 genuine (57 `Kerastase`→`Kérastase` accent, 7 `myBoostars`→`my Boostars` spacing confirmed via packaging image, `&herb` × 2 resolved from `BRD-UNDEFINED` by registering new brand `BRD-SG-13508`, 2 `Loreal`/`L'Oreal` brand_id-fragmentation rows rerouted to the established `BRD-GLOBAL-00015`; 15 `BRD-UNBRANDED` rows confirmed correct false positives), `excess_content`/`canonical_field_mismatch` (multi-component bundle text cleanup, a genuinely mis-conflated multi-pack-count SKU split — `SKU-069640` covering 1×/2×/3× 620ml Head & Shoulders listings under one `pack_count=3` entry — into 3 entries), `null_size` 78 (69 resolved: the SKU-002xxx keyword-seed brand-catch-all family set to `is_multi_size=TRUE`/`size=NULL` for internal consistency, after first wrongly sweeping in 17 genuine single-product Dove/Sunsilk/Tresemme entries via an overbroad `product_line='Shampoo'` match — caught via fan-out check before it shipped, reverted, sizes restored from their own `canonical_name` text), `garbage_brand` 2 (brand `"107"`/`"1o7"` — confirmed genuinely printed on packaging via image read, not a reseller watermark, no fix needed). STEP 2b promo-language sweep (321 pack_count=1 rows with promo/multiplier text) plus a GMV-prioritized Tier 2 sample surfaced real pack-count misses beyond Tier 1's reach: KUNDAL/Himalaya/NARD/andSons/Lakme/Pantene/Schwarzkopf/Grafen buy-N-get-1 and explicit `xN`/`[Bundle of N]` listings with `pack_count` stuck at 1 — fixed 20+ rows, including splitting 8 more taxonomy entries that conflated multiple distinct bundle sizes (e.g. Pantene `[Bundle of 2]` vs `[Bundle of 3]`, Schwarzkopf Ginger `Bundle of 2/3/4`) under one entry. Also found and fixed a systemic **truncated-canonical-name bug affecting 38 Grafen entries** (unclosed `[` marketing-claim bracket concatenated directly onto the trailing size, e.g. `"...54. 500ml x3"` missing the closing bracket and rest of the claim) — stripped the marketing text, synced `product_line`, cleaned residual `"300/"` and stray `"Bundle of N]"` fragments from a second corruption pattern. Highest-GMV single fix: `SKU-069510` (NARD Hair Mini Sample, SGD ~10.2M GMV) had `size`/`product_line` NULL despite the image showing a clearly-labeled 10-sachet (5 shampoo + 5 treatment formula) 8ml sampler set — set `size='8ml'`, `pack_count=10`, `is_multi_variant=TRUE`. Minted 10 new entries (`SKU-133846`–`SKU-133855`, claimed 200-slot block `SKU-133846`–`SKU-134045`) for products that needed splitting off a wrongly-conflated entry rather than an in-place fix. 553 taxonomy entries had content fixed (`_meta` reset to unreviewed for re-check next session); 29 entries were genuinely Tier-2-judged and found already correct (marked `confident`/`unconfident` per agreement with prior verdict). | QA gates re-verified live post-write: G1 dual-mapped 0, G2 HUMAN+LLM coexistence 0, G5 provenance 0, G6 brand_mismatch 0, placeholder-leak 0 (down from 182 distinct entries carrying the banned phrase, entirely closed out this session — see the 2026-07-23 top-up session above for how that debt was flagged and deferred). Universe refresh **not run this session** — explicitly out of scope per instructions. SKU block `SKU-133846`–`SKU-134045` left `ACTIVE` (10/200 used, reservation not a quota). Confidence distribution left behind: 553 entries reset to unreviewed (re-checked fresh next session), 29 marked confident/unconfident, remainder of the ~2,387 distinct entries in this category unchanged from prior state. Residual precision debt for a future session: the ~1,795 Tier-1-clean rows not sampled in this session's bounded Tier 2 GMV pass remain exactly as eligible as before (per design, no fabricated review) — Tier 1's cheap sweep will re-catch any mechanical defects among them for free next run. |
