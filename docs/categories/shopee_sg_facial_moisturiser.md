...
> used everywhere else in this pipeline — the shorthand `shopee_sg_facial_moisturiser` used in STATUS.md and
> task prompts is not the literal table name, same naming gap already documented in
> `docs/categories/shopee_sg_facial_cleanser.md`).
...
method already validated in `docs/categories/shopee_sg_facial_cleanser.md`). `BRD-UNDEFINED` (unresolved
...
sibling `shopee_sg_facial_cleanser` session (2026-05-01) rather than the newer 2026-06-01, which is
...
already flagged in `shopee_sg_facial_cleanser.md`'s Brand Scope footnote, confirmed independently in this
...
SG multi-brand retailers in `shopee_sg_facial_cleanser.md` — confirms the pattern is category-independent.
...
(they aren't there yet — no SG category had been LLM-extracted before `shopee_sg_shampoo`/`shopee_sg_facial_cleanser`).
...
|---|---|---|
| `belif Official Store` | belif, CNP LABORATORY | LG H&H (confirmed in `shopee_sg_facial_cleanser.md`) |
...
`shopee_sg_facial_cleanser.md` applied to single-listing "Official" stores): `Bio Essence Official Store`
...
conclusion as `shopee_sg_facial_cleanser.md`. Method: text-first clustering into canonical entries (see
...
differs from `shopee_sg_facial_cleanser`'s own cleanser bucket; a men's cleanser product could in principle
...
table's Men's Care buckets (i.e. don't reroute mainstream cleansers here — `shopee_sg_facial_cleanser` owns
those).
...
- `image` column has the same literal-embedded-double-quote data-quality quirk documented in
  `shopee_sg_facial_cleanser.md` (e.g. `.../file/"sg-11134207-...` — quotes are literal characters in the
...
precedent in `docs/categories/shopee_sg_facial_cleanser.md`.
...
("shopee_sg_facial_moisturiser: ⏳ Keyword only") — these are automated keyword-seed rows (the `HUMAN` label
