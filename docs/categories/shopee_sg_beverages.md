...
| 2026-07-29 | Top-up bulk match | Grouped the 308-row worklist by brand text-matched against this category's 1,448 existing taxonomy entries (structured `brand_id`/`size`/`pack_count` join, not canonical_name similarity). 91 products reused an existing entry (exact brand+size+pack match, incl. Evian/Volvic "Assorted Sizes"/"Variety" catch-alls for 2 genuinely multi-size listings). 212 products needed a new entry, consolidated into 208 new taxonomy entries (2 real duplicate-listing pairs merged; the rest are genuinely distinct long-tail products — mostly Chinese/Korean herbal teas, juices, and niche imports at the 91–95% cumulative-GMV tail). 5 products (supplement capsules/tablets: FIQ Herbs, Risell, Allre, Okinawa Chosei; and raw Pearl Barley grain) are non-beverage and left `taxonomy_id NULL` per category scope | Bulk `INSERT...VALUES` DML: 208 new `product_taxonomy` rows (SKU-210049–SKU-210256, all `meta_agent='CLAUDE_CODE'`) + 303 new `product_taxonomy_map` rows (91 reuse + 212 new), all `source='LLM'`, `platform='Shopee'`, `country='SG'`. No image reads performed — text-only bulk extraction, consistent with Full Rebuild top-up philosophy (coverage over precision; exact `product_line` wording deferred to `targeted_qa_fix.sh`) |
| 2026-07-29 | Top-up post-run QA gates | Re-ran STEP 0 verbatim post-write: gap fell from 308 to exactly the 5 intentional-NULL non-beverage products, confirming full resolution of the addressable worklist. Gates: G1 dual-mapped(LLM)=0, G2 HUMAN+LLM coexistence=0 (checked without `--skip-coexistence`), placeholder-leak=0, structured-fields (product_line NULL%) among LLM=0%, provenance(NULL meta_agent/source)=0. Category-total LLM map rows now 1,975 (was 1,672) | All gates pass; universe refresh eligible (not run this session — separate step) |
