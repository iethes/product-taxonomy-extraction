...
| 2026-07-28 | Fifth top-up coverage | **Registry bug found and fixed before any taxonomy write**: the first atomic block claim (`DECLARE`/`BEGIN TRANSACTION` per the runbook) computed `next_start` from `COALESCE(MAX(block_end),...)` and landed at `192544`–`192941` — identical to the fourth top-up session's already-used range, which has 63 real `product_taxonomy` rows (`SKU-192544`–`SKU-192606`) already in it. Root cause: the fourth top-up session's own block claim was never written to `sku_block_registry` (a real gap in that session's own record-keeping, not something introduced here) so the registry's own `MAX(block_end)` for this category was stale at `192543`. Caught by cross-checking the freshly-claimed range against `product_taxonomy` row counts before writing anything — found 63 existing rows in the "empty" claimed range. Fix: deleted the bad claim row, backfilled a corrective historical registry row for the fourth top-up's actual `192544`–`192941` range, then re-claimed fresh — landed at `193342`–`193361` (the gap between `192941` and `193342` reflects a *different* category's parallel claim in between, confirmed via the registry's other rows, not a further bug) | Registry corrected; claimed a verified-clean 20-slot block (`SKU-193342`–`193361`) sized to this session's actual 5 new entries, not the prompt's literal 297, since 297 was never going to be the count of *new mints* — see `docs/headless-runbook.md`'s Shared Mechanics section, which itself flags `sku_block_registry` has no equivalent atomic-claim protection issue for `brand_dict` but doesn't call out that a *missing* registry row for a real claim is possible; flagging that gap for a future runbook update |
...
| NULL (out-of-scope, correctly excluded, this session's live worklist) | 0 | This session's worklist was already scoped to `taxonomy_id IS NULL` in-scope rows only (OOS rows are excluded by the STEP-0 query's join, not counted separately here) |
