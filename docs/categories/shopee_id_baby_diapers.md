...
`shopee_th_body_wash`/`shopee_th_liquid_milk`).
...
| 2026-07-25 16:54 UTC | Automated review session (auto-discovery) | Auto-discovery Targeted QA Fix. Pre-fix gate report flagged 75 'all variant/size name' violations (no qa_gate_exceptions on file for this category — genuine defects, not false positives): all were the banned 'Multiple Sizes'/'Multiple Variants' suffix on is_multi_size=TRUE catch-all entries created before the 2026-07-22 unconditional-ban rule. Tier 1 SQL sweep over the full worklist (3,077 entries) also found 216 entries with brand_dict casing mismatches (canonical_name used 'POKANA'/'Goo.N' instead of brand_dict's 'Pokana'/'Goo.n'). STEP 2b's pack_count=1-plus-promo-language sweep (3,590 model-grain rows, 34 distinct products) found this category's pack_count field is semantically inconsistent across pre-existing entries: some (e.g. SKU-093462, SKU-094127) correctly store total pieces (per-bag count x bag count, verified via exact arithmetic against sku_name text and confirmed via image), while others silently defaulted to pack_count=1 when a multi-bag/carton quantifier ('N Ball', 'PAKET N BAL', 'KARTON') wasn't parsed. Verified via curl+Read product images for the top-GMV candidates before concluding this (2 MAKUKU 'N PACK' banner images and 1 Merries jumbo-box image confirmed the true multiplier; 1 MAKUKU 'isi 3' image and 1 image showing the listing was genuinely single-pack confirmed pack_count=1 was already correct for those two). Self-correction note: this session's first pass at fixing 4 of these misses incorrectly used a bag-count convention (x2/x4) instead of the taxonomy's actual total-piece-count convention; caught via an advisor consult before finalizing and corrected in place before any universe refresh. A broader GMV-ranked scan for the same 'karton\|bal\|ball\|dus\|renceng' mis-route pattern surfaced ~40 more candidate rows in the top-GMV slice alone; only the top 5 by GMV were hand-fixed per the advisor's bounded-scope guidance (all reused pre-existing correctly-numbered entries, no new SKUs needed) — the remaining ~35+ candidates in this scan, plus whatever exists below it, are a real, likely large-GMV-impact systemic gap flagged for a dedicated future session, not something this targeted pass should chase exhaustively. Also found (not actioned, out of scope): several pre-existing duplicate taxonomy entries sharing an identical canonical_name+brand+is_multi_size (e.g. three separate 'Sweety Silver Pants' catch-all entries) — cannot be merged without deleting a row, which this session type never does. | Fixed 75 banned-phrase canonical names via bulk REGEXP_REPLACE (strips ' Multiple Sizes'/' Multiple Variants' suffix, leaving brand+line only per the established Fluffy/Baby Happy precedent for entries with no distinguishable product line). Fixed 216 brand-casing mismatches via bulk prefix replace. Corrected pack_count on 20 products: reused 15 pre-existing correctly-numbered taxonomy entries (including catch-all is_multi_size=TRUE entries for cases where no per-bag piece count was textually derivable — routed there rather than guessing a number), and repurposed 3 of this session's own newly-inserted entries in place once their content was corrected (SKU-096559 to x160, SKU-096560 to x48, SKU-096569 to x24). 9 of this session's 12 inserted taxonomy rows ended up orphaned (0 map rows) as a byproduct of the self-correction — left in place per the never-delete rule, documented here so a future session isn't confused by them. All 9 hard-gate checks (G1 dual-mapped, G2 HUMAN+LLM coexistence, placeholder-leak, structured-fields NULL% at 35%, duplicate product_id, duplicate product+taxon, 'all variant/size' name, canonical_name fields, garbled brand text) pass at 0/within-threshold, re-verified after all fixes without --skip-coexistence. GMV coverage essentially unchanged (98.38% to 98.39%, as expected for a quality-only pass). 3 entries (SKU-093462, SKU-094127, SKU-094399) received a genuine Tier 2 'correct' verdict this session (first review, so _meta lands on unconfident per the two-agreeing-reviews rule) after their pack_count was arithmetic-verified against sku_name text. No universe refresh run (out of scope per STEP 8). |
---
