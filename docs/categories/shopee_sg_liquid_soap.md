...
| Source | Before Full Rebuild | After Full Rebuild | After top-up sessions #1–#4 | After top-up session #5 (2026-07-23) | Notes |
|--------|-----------------|-----------------|-----------------|-----------------|-------|
| LLM | 0 | 1,948 | 2,228 | 2,702 (verified via live `COUNT(*) GROUP BY source`) | 686 Pass 1 + 1,262 Pass 2 + 267 top-up #1 + 7 top-up #2 + 3 top-up #3 + 0 top-up #4 + 477 top-up #5 (246 new-entry + 231 reused) — running total is directionally consistent with the verified 2,702, small variance from map-row/product_id collapsing across sessions, not investigated further |
| HUMAN | 1,726 | 1,726 | 1,726 (flagged unresolved every session #1–#4) | **0** | An external process deleted all 1,726 legacy HUMAN rows between session #4 and session #5 — not done by this or any documented session. This resolves the long-flagged G2 open item, but is also the root cause of session #5's much larger live gap (802 products vs. sessions #2–#4's 277–394) — products whose only taxonomy had been a HUMAN row lost their mapping when it was deleted. |
| In-scope set (month 2026-06 top-up worklist) | — | — | Sessions #1–#4 combined: 289 products newly mapped, remainder of each re-query confirmed as recurring bar-soap/wrong-format bleed (see QA History rows above) | Session #5: live re-query found 1,487 rows / 802 distinct products ($436,460 GMV row-sum) — 509 were genuine liquid-format coverage misses (mostly re-exposed by the HUMAN-row deletion), 477 successfully mapped (246 new-entry + 231 reused), 32 left NULL as ambiguous/unresolvable; 293 confirmed OOS bar-soap/wrong-format bleed consistent with sessions #1–#4's documented pattern | See QA History for the full breakdown |
| GMV coverage (all products, 2026-06) | — | 94.5% | not re-measured in sessions #1–#4 | not re-measured this session either (`marketshare_universe` schema doesn't expose `master_table` for a quick coverage query; worklist was Rule-A-only, not the full §2 in-scope set) | — |
