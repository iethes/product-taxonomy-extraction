...
|--------|-----------------|-----------------|-----------------|-------|
| LLM | 0 | 1,948 | 2,228 (after session #3) | 686 Pass 1 + 1,262 Pass 2 + 267 top-up #1 (24 reused-entry + 243 new-entry) + 7 top-up #2 (2 reused + 5 new-entry) + 3 top-up #3 (0 reused + 3 new-entry) |
| HUMAN | 1,726 | 1,726 | 1,726 | Untouched across all sessions — no HUMAN dedup/delete step run yet |
| In-scope set (month 2026-06 top-up worklist) | — | — | Session #1: 664 rows re-queried, 273 mapped, 391 confirmed OOS. Session #2: live re-query found 394 rows/288 products ($56,386 GMV) — 281 products (~98.5% of GMV) reconfirmed as the same already-adjudicated OOS set; 7 products (~$814 GMV) were genuine new misses, now mapped. Session #3: live re-query found 384 rows/280 products ($55,473 GMV) — 275 products (98.2%) reconfirmed as the same already-adjudicated OOS set; 5 products were genuine liquid-format candidates, 3 mapped (new entries), 2 left NULL (size unresolvable from available signals). Session #4: live re-query found 377 rows/277 products ($54,850 GMV) — 0 genuine new misses; entire gap is the same already-adjudicated bar-soap/wrong-format bleed plus 2 known-NULL products (MUJI, Clarins) already flagged in session #3 as size-unresolvable. | See QA History for the full OOS breakdown |
