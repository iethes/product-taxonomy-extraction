...
| 2026-07-28 17:37 UTC | Automated review session (auto-discovery) | Worklist of 129 never-confident entries. STEP 1C fast-lane bulk-promoted 18 'fixed pending recheck' rows (Tier-1-clean, needed one more confirming review). Tier 1 SQL sweep (STEP 2/2b/2c) found zero mechanical defects on the remaining 111 — this category has already been through 2 deep targeted-QA passes and a top-up session. Tier 2 review used the bulk sku_name-vs-canonical_name text-comparison method (this category's own established precedent) across the full remaining 111-row worklist, escalating to product images only for genuinely ambiguous/high-GMV cases (11 image reads total). This text-only method proved unreliable in both directions this session — SKOL product 5271137790's current sku_name read 'Skol Beer Can (24x320ml)' but its packaging photo confirmed it is genuinely 'Skol Strong 8.0% 24x490ml' (a stale/glitched title, no fix needed); conversely Kronenbourg 20495920611's month-aggregated title suggested a 24-count mismatch that vanished once checked against the current month alone (also no fix needed). Both were caught only because they were image/month-checked; the other 101 rows promoted to confident via Path 2 this session were judged on current-month sku_name text only, not image-verified — flagging this for any future session that treats 'confident' as image-verified. Genuine defects found and fixed: (1) Asahi 'Super Dry 350ml x24' (2nd-highest GMV in sample, 8,787 SGD) was 49% contaminated — 1 Kuronama product (different line) and 3 Dry Zero non-alcoholic products (different product entirely) silently merged in from the prior Full Rebuild's bulk text-matching, confirmed via product images (packaging text '350ml x24缶入' on 2 of 3 Dry Zero listings). (2) Carlsberg 'Danish Pilsner 320ml x12' had ZERO genuine x12 products — both its mapped products were actually Bundle-of-2 and Bundle-of-4 multiples of a 12-can base (24 and 48 total), confirmed via packaging images showing 'BUNDLE OF 2'/'BUNDLE OF 4' badges on a 12x320ml base carton. (3) Somersby 'Passionfruit & Orange Cider 330ml x4' had 2 of 3 products actually Pear Cider and Apple Cider (variant collapse, same contamination class as Asahi). (4) Weihenstephaner 'Hefeweissbier 500ml x20' had 1 of 2 products actually the Dunkel (dark) variant. (5) Kilkenny '440ml x24' included a product whose packaging image explicitly reads '8 PACK 440ML', not part of a 24-count. (6) Kirin Ichiban '350ml x24' included 'Kirin Ichiban Free: Non-Alcoholic & Zero Sugar' — a genuinely different (non-alcoholic) product with no pack count ever stated in its sku_name history or a legible count in its image (a 3-product marketing collage of different SKUs, not this listing's actual packaging) — routed to a new entry but NOT sealed confident, since the x24 pack count assigned to it is an unevidenced assumption-by-analogy, not a confirmed value. (7) Hite 'Extra Cold 355ml x24' had a product duplicating an already-existing dedicated 'Hite Extra Cold 355ml x24' entry (SKU-112729) instead of being routed there. (8) Two entries (Suntory Horoyoi Chu-Hi, TTL Taiwan Beer) had is_multi_variant=FALSE despite sku_name/image evidence of genuine buyer-choice multi-flavor listings (TTL confirmed via image: '10 Flavors, 6x330ml' assortment box) — TTL's canonical_name also carried the ungrounded 'Assorted Flavours' catch-all text alongside the flag, same defect class as this category's already-fixed '(all variants)' stubs from a prior session; corrected to null product_line + flag only, per that precedent. | Claimed SKU block SKU-195049-SKU-195248 (targeted_qa_fix). Minted 8 new entries: SKU-195049 Asahi Kuronama 350ml x24, SKU-195050 Asahi Dry Zero 350ml x24, SKU-195051 Carlsberg Danish Pilsner 320ml x48, SKU-195052 Somersby Pear Cider Pint 330ml x4, SKU-195053 Somersby Apple Cider Pint 330ml x4, SKU-195054 Weihenstephaner Dunkel Hefeweissbier Dark Wheat Ale 500ml x20, SKU-195055 Kirin Ichiban Free Non-Alcoholic Zero Sugar 350ml x24 (left unconfident/fixed-pending-recheck — pack_count=24 is an unverified analogy, not confirmed text/image evidence, so it was deliberately not sealed confident despite passing Tier 1), SKU-195056 Kilkenny Irish Red Ale Draught 440ml x8. Rerouted 12 products: 4 to new Asahi entries, 1 to new Carlsberg x48 entry, 1 existing-Carlsberg-x12 product to the existing SKU-116613 (x24), 2 to new Somersby entries, 1 to new Weihenstephaner entry, 1 to new Kirin entry, 1 to new Kilkenny entry, 1 Hite product to the existing SKU-112729. Fixed is_multi_variant on SKU-112742 (Suntory Horoyoi, flag-only fix) and SKU-112736 (TTL Taiwan Beer, flag fix + stripped 'Assorted Flavours' generic text from product_line/canonical_name, renamed to 'TTL Taiwan Beer 330ml x6'). All touched/new entries re-checked against Tier 1 immediately post-fix (same-session gate-verify); all came back clean except SKU-195055, deliberately left at fixed-pending-recheck. Left unresolved, flagged for a future session: DONKI listing 28021229515 (GMV 362, genuinely spans Super Dry 350/500ml + Kuro Nama in one buyer-choice listing, left on Asahi Super Dry 350x24 as least-bad default per this category's own precedent for this exact ambiguity class); the 3 Somersby Ninja-Slushi bundle entries (SKU-112635/112643/112644, GMV 0 this review month, genuinely confusing 'N x product / Pack of 24' phrasing never resolved); SKU-112684 (Carlsberg Danish Pilsner 320ml x12) is now orphaned at 0 mapped products after both its former products were rerouted elsewhere — left as-is per no-deletion constraint. Confidence left behind: 168 confident (18 via STEP 1C fast-lane, 19 via Path 1b same-session fix-and-clean, 101 via Path 2 bulk text-review agreeing with prior verdict, 30 pre-existing from before this session), 17 unconfident, 1 fixed-pending-recheck (SKU-195055). Hard gates: G1 dual-mapped=0 ✅, G2 HUMAN+LLM coexistence=0 ✅, placeholder-leak=0 ✅, structured-fields NULL%=12% (well under 50% gate) ✅, G5 provenance=0 ✅. |
| 2026-07-28 17:51 UTC | Automated review session (auto-discovery) | Worklist of 18 never-confident entries (STEP 1C found 1 fixed-pending-recheck row: SKU-195055). Tier 1 SQL sweep (STEP 2/2b/2c) found zero mechanical defects on all 18. Went beyond Tier 1 and applied Tier 2 GMV-prioritized bulk sku_name-vs-canonical_name comparison (current month 2026-06-01 only, this category's own established precedent) across the full remaining worklist, since it was small enough to review exhaustively. This surfaced 3 fresh contamination/miscount defects the prior 3 sessions' sweeps hadn't caught, all confirmed via product image: (1) SKU-112606 'Carlsberg Danish Pilsner Football Edition 320ml x24' had drifted again since the last session's cleanup — 2 of 3 mapped products (24863338335, 56707521602) currently read plain 'Danish Pilsner Green Label' with no Football Edition text, leaving only 1 genuine Football Edition product. (2) SKU-112649 'Heineken Lager 320ml x24' had 3 contaminated products: 14485256897 is genuinely 'Heineken Silver' (confirmed via on-can text, a distinct product line, not Lager), 25517601535 states '(2 Cartons)' which the product image confirmed as 2x24=48 not 24, and 28956456565 states '10 Cartons' which its image explicitly confirms as '240 CANS' wholesale — both miscounted at pack_count=24. (3) SKU-112743 'Tiger Lager 320ml x24' had 1 contaminated product (4111461321), confirmed via image to be 'Tiger Crystal Cold Filtered Lager' (on-can text), a distinct product line from plain Tiger Lager and distinct from the existing SKU-116608 Tiger Crystal x10 entry (different pack count). Also re-verified SKU-112611's known pre-existing multi-brand concatenated-listing ambiguity (Tuborg Strong / Danish Pilsner) — still present, still unresolvable without deletion authority, left as-is per prior session's documented precedent. Independently re-checked SKU-195055's flagged pack_count=24 evidence gap: fetched its product image directly — confirmed it is a 3-product marketing collage (Green's Free / Zero Ichi / Kara Free), not the listing's actual packaging, so pack_count remains genuinely unconfirmed; deliberately did not bulk-promote it via STEP 1C despite passing Tier 1, since that would paper over a real, previously-flagged evidence gap. | Claimed SKU block SKU-195449-SKU195648 (targeted_qa_fix). Minted 3 new entries: SKU-195449 Heineken Silver 320ml x24, SKU-195450 Heineken Lager 320ml x240, SKU-195451 Tiger Crystal Cold Filtered Lager 320ml x24. Rerouted 6 product_taxonomy_map rows: 2 products from SKU-112606 to the existing plain-Danish-Pilsner entry SKU-116613, 1 product from SKU-112649 to new SKU-195449, 1 product from SKU-112649 to the existing SKU-112651 (x48), 1 product from SKU-112649 to new SKU-195450, 1 product from SKU-112743 to new SKU-195451. All touched/new entries re-checked against Tier 1 immediately post-fix (same-session gate-verify) — all clean. Confidence promotion: 19 entries promoted to confident in one bulk statement (3 new entries + 3 fixed entries via Path 1(b) same-session clean recheck, 12 unresolved-but-Tier-1-clean entries via Path 2 agreement with their prior 'correct' verdict — SKU-112608, SKU-112610, SKU-112651, SKU-112750, SKU-141043 through SKU-141051 excluding 048/049 typo). Left SKU-112611 (pre-existing multi-brand listing ambiguity, needs deletion authority) and SKU-195055 (pack_count evidence gap confirmed unresolvable from available signals) unresolved. Category-wide confidence state after this session: 187 confident, 1 unconfident (SKU-112611), 1 fixed-pending-recheck (SKU-195055) — every other entry mapped to this table is now confident. Hard gates (self-checked, no --skip-coexistence): G1 dual-mapped=0 ✅, G2 HUMAN+LLM coexistence=0 ✅, placeholder-leak=0 ✅, structured-fields NULL%=12% (well under 50% gate) ✅, G5 provenance=0 ✅, duplicate product_id=0 ✅. |
---
